...
int main(int argc, char *argv[])
{
    ...
    // Handshake name fixed to X25519/ChaChaPoly/BLAKE2s
    ...
    uint8_t server_private[32], server_public[32], client_public[32];
    if (load_key_fixed("./Keys/server_priv.txt", server_private, 32)) return 1;
    ...
    if(setsockopt(socket_desc, SOL_SOCKET, SO_REUSEADDR, &(int){1}, sizeof(int))<0) { perror("setsockopt"); return 1; }
    memset(&server, 0, sizeof(server));
    server.sin_family = AF_INET; server.sin_addr.s_addr = INADDR_ANY; server.sin_port = htons(PORT);
    if(bind(socket_desc,(struct sockaddr *)&server , sizeof(server))<0) { perror("bind"); return 1; }
    if(listen(socket_desc , 16)<0) { perror("listen"); return 1; }
    ...
    for(int i=0;i<iters;i++){
        ...
        err = noise_handshakestate_new_by_name(&handshake, to_test_full, NOISE_ROLE_RESPONDER);
        ...
        while (ok) {
            ...
            if (action == NOISE_ACTION_WRITE_MESSAGE) {
                ...
                while(full>0){
                    ...
                    ssize_t w = send(new_socket, message+sent, chunk, 0);
                    if (w<0){ ok=0; break; }
                ...
                while(received<need){
                    ssize_t r = recv(new_socket, message+received, sizeof(message)-received, 0);
                    if (r<=0){ ok=0; break; }
                ...
                if (assert_dynamic) {
                    ...
                    if (herr == NOISE_ERROR_NONE) {
                        fprintf(stderr, "[server] iter=%d h[0..7]=%02x%02x%02x%02x%02x%02x%02x%02x\n",
                                i, hbuf[0],hbuf[1],hbuf[2],hbuf[3],hbuf[4],hbuf[5],hbuf[6],hbuf[7]);
                        if (last_hlen && memcmp(last_h, hbuf, last_hlen)==0) {
                            fprintf(stderr, "[WARN] server: handshake hash unchanged vs last iter — RNG/fixed-e suspected\n");
                        }
        ...
        if (i >= warmup) {
            printf("%s%c%s%c%s%c%s%c%d%c%.3f%c%d\n",
                   "server", sep, pattern, sep, kem_print, sep, label, sep, i, sep, ms, sep, ok?0:1);
            fflush(stdout);
        }
