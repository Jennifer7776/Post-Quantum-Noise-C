// ===============================
// server_pq2.c — Synchronization logic with the client: Provide static/temporary PQ keys (TCP) as needed for the handshake.
// Build: gcc -O2 -Wall -Wextra server_pq.c -lnoise -loqs -o server_pq
...
static int role_char_responder(const char *pattern){
    if (!pattern || !pattern[0]) return 'N';
    int c = (unsigned char)( pattern[1] ? pattern[1] : pattern[0] );
    if (c>='a'&&c<='z') c-=32;
    return c;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 3; i < argc; i++) {
        ...
        else if (!strcmp(argv[i],"--bind")   && i+1<argc)  bindip = argv[++i];
    ...
    // Fixed (classic) 32B (if the library will use it internally)
    uint8_t server_priv[32], server_pub[32], client_pub_x25519[32];
    if (load_key_fixed("./Keys/server_priv.txt", server_priv, 32)) return 1;
    if (load_key_fixed("./Keys/server_pub.txt",  server_pub,  32)) return 1;
    if (load_key_fixed("./Keys/client_pub.txt",  client_pub_x25519,  32)) return 1;

    // Peer-side static PQ public key (if needed)
    uint8_t *client_pub_pq=NULL; size_t client_pub_pq_len=0;
    load_pq_key_multi("client_pub", kem_arg, &client_pub_pq, &client_pub_pq_len); // 允许失败

    // Static PQ private/public key (if needed)
    uint8_t *server_priv_pq=NULL,*server_pub_pq=NULL;
    size_t server_priv_pq_len=0, server_pub_pq_len=0;
    load_pq_key_multi("server_priv", kem_arg, &server_priv_pq, &server_priv_pq_len);
    load_pq_key_multi("server_pub",  kem_arg, &server_pub_pq,  &server_pub_pq_len);
    ...
    if (inet_pton(AF_INET, bindip, &addr.sin_addr)!=1){ close(ls); return 1; }
    ...
    const int rc = role_char_responder(pattern); // Determine whether the local end should have a static key (K/I)
    const int have_static = (rc=='K' || rc=='I') && server_priv_pq && server_pub_pq;
    ...
    for(int i=0;i<iters;i++){
        ...
        int err = noise_handshakestate_new_by_name(&hs, proto_name, NOISE_ROLE_RESPONDER);
        if (getenv("PQ_WHITEBOX")) fprintf(stderr, "[PQWB] proto=%s err=%d\n", proto_name, err);
        if (err!=NOISE_ERROR_NONE){ close(cs); continue; }

        while (noise_handshakestate_needs_local_keypair(hs)) {
            err = supply_local_keypair(hs, have_static, server_priv_pq, server_priv_pq_len,
                                              server_pub_pq,  server_pub_pq_len, oqs_name);
            if (err!=NOISE_ERROR_NONE) break;
        }
        if (err!=NOISE_ERROR_NONE){ noise_handshakestate_free(hs); close(cs); continue; }

        // If need a remote static PQ public key, please provide it (I will continue to provide it in cycles later).
        if (noise_handshakestate_needs_remote_public_key(hs) && client_pub_pq && client_pub_pq_len){
            NoiseDHState *rdh = noise_handshakestate_get_remote_public_key_dh(hs);
            err = noise_dhstate_set_public_key(rdh, client_pub_pq, client_pub_pq_len);
            if (err!=NOISE_ERROR_NONE){ noise_handshakestate_free(hs); close(cs); continue; }
        }
        ...
        while (ok) {
            ...
            if (action == NOISE_ACTION_READ_MESSAGE) {
                size_t need=2, got=0;
                while (got<need) {
                    ssize_t r = recv(cs, message+got, need-got, 0);
                    if (r<=0){ ok=0; break; }
                    got += (size_t)r;
                ...
                while (noise_handshakestate_needs_local_keypair(hs)) {
                    int err2 = supply_local_keypair(hs, have_static, server_priv_pq, server_priv_pq_len,
                                                          server_pub_pq,  server_pub_pq_len, oqs_name);
                ...
                if (noise_handshakestate_needs_remote_public_key(hs) && client_pub_pq && client_pub_pq_len){
                    NoiseDHState *rdh = noise_handshakestate_get_remote_public_key_dh(hs);
                    int err2 = noise_dhstate_set_public_key(rdh, client_pub_pq, client_pub_pq_len);
                    if (err2!=NOISE_ERROR_NONE){ ok=0; break; }
                }
                ...
                while (sent < total) {
                    ...
                    ssize_t w = send(cs, message+sent, chunk, 0);
                    if (w<=0){ ok=0; break; }
                    sent += (size_t)w;
                }
                if (!ok) break;
        ...
        if (i>=warmup)
            printf("server%c%s%c%s%c%s%c%d%c%.3f%c%d\n",
    ...
    free(client_pub_pq);
    free(server_priv_pq); free(server_pub_pq);
    return 0;
}
