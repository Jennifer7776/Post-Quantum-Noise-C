...
/* —— 生成密钥对 —— */
static int noise_hqc192_keypair(NoiseDHState *state, const NoiseDHState *other)
{
    NoiseHqc192State *st = (NoiseHqc192State *)state;
...
static int noise_hqc192_set_keypair_private(NoiseDHState *state, const uint8_t *private_key)
{
...
/* —— 成对设置 pk/sk —— */
static int noise_hqc192_set_keypair(NoiseDHState *state, const uint8_t *private_key, const uint8_t *public_key)
{
    NoiseHqc192State *st = (NoiseHqc192State *)state;
...
/* —— 基础公钥校验：至少拒绝全零 —— */
static int noise_hqc192_validate_public_key(const NoiseDHState *state, const uint8_t *public_key)
{
...
static int noise_hqc192_calculate(const NoiseDHState *private_key_state,
...
/* —— Encap/Decap —— */
static int noise_hqc192_encapsulate(const NoiseDHState *state, uint8_t *cipher, uint8_t *shared)
{
    NoiseHqc192State *st = (NoiseHqc192State *)state;
...
static int noise_hqc192_decapsulate(const NoiseDHState *state, const uint8_t *cipher, uint8_t *shared)
{
    NoiseHqc192State *st = (NoiseHqc192State *)state;
...
/* —— 构造器：HQC192 —— */
NoiseDHState *pqnoise_hqc192_new(void)
{
    NoiseHqc192State *state = noise_new(NoiseHqc192State);
    if (!state) return 0;

    state->kem = OQS_KEM_new(OQS_KEM_alg_hqc_192);
    ...
    state->parent.dh_id = NOISE_DH_HQC192;
    state->parent.nulls_allowed = 0;

    state->parent.private_key = state->private_key;
    state->parent.public_key  = state->public_key;

    state->parent.generate_keypair     = noise_hqc192_keypair;
    state->parent.set_keypair          = noise_hqc192_set_keypair;
    state->parent.set_keypair_private  = noise_hqc192_set_keypair_private;
    state->parent.validate_public_key  = noise_hqc192_validate_public_key;
    state->parent.copy                 = NULL; 
    state->parent.calculate            = noise_hqc192_calculate;
    state->parent.encaps               = noise_hqc192_encapsulate;
    state->parent.decaps               = noise_hqc192_decapsulate;
