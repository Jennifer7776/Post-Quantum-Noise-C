...
/* — Generate key pair — */
static int noise_bikel3_keypair(NoiseDHState *state, const NoiseDHState *other)
{
    NoiseBikeL3State *st = (NoiseBikeL3State *)state;
...
static int noise_bikel3_set_keypair_private(NoiseDHState *state, const uint8_t *private_key)
{
...
static int noise_bikel3_set_keypair(NoiseDHState *state, const uint8_t *private_key, const uint8_t *public_key)
{
    NoiseBikeL3State *st = (NoiseBikeL3State *)state;
...
static int noise_bikel3_validate_public_key(const NoiseDHState *state, const uint8_t *public_key)
{
...
static int noise_bikel3_calculate(const NoiseDHState *private_key_state,
...
/* —— Encap/Decap —— */
static int noise_bikel3_encapsulate(const NoiseDHState *state, uint8_t *cipher, uint8_t *shared)
{
    NoiseBikeL3State *st = (NoiseBikeL3State *)state;
...
static int noise_bikel3_decapsulate(const NoiseDHState *state, const uint8_t *cipher, uint8_t *shared)
{
    NoiseBikeL3State *st = (NoiseBikeL3State *)state;
...
/* —— Constructor：BIKEL3 —— */
NoiseDHState *pqnoise_bike_l3_new(void)
{
    NoiseBikeL3State *state = noise_new(NoiseBikeL3State);
    if (!state) return 0;

    state->kem = OQS_KEM_new(OQS_KEM_alg_bike_l3);
    ...
    state->parent.dh_id = NOISE_DH_BIKEL3;
    state->parent.nulls_allowed = 0;

    state->parent.private_key = state->private_key;
    state->parent.public_key  = state->public_key;

    state->parent.generate_keypair     = noise_bikel3_keypair;
    state->parent.set_keypair          = noise_bikel3_set_keypair;
    state->parent.set_keypair_private  = noise_bikel3_set_keypair_private;
    state->parent.validate_public_key  = noise_bikel3_validate_public_key;
    state->parent.copy                 = NULL; 
    state->parent.calculate            = noise_bikel3_calculate;
    state->parent.encaps               = noise_bikel3_encapsulate;
    state->parent.decaps               = noise_bikel3_decapsulate;
