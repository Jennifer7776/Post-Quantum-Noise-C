...
/* —— 生成密钥对 —— */
static int noise_hqc_generate_keypair
        (NoiseDHState *state, const NoiseDHState *other)
{
    NoiseHQCState *st = (NoiseHQCState *)state;
...
static int noise_hqc_set_keypair_private
...
/* —— 成对设置 pk/sk —— */
static int noise_hqc_set_keypair
...
/* —— 基础公钥校验：至少拒绝全零 —— */
static int noise_hqc_validate_public_key
...
static int noise_hqc_calculate
...
/* —— Encap/Decap —— */
static int noise_hqc_encapsulate
...
static int noise_hqc_decapsulate
...
/* —— 构造器：HQC-128 —— */
NoiseDHState *pqnoise_hqc128_new(void)
{
    NoiseHQCState *state = noise_new(NoiseHQCState);
    if (!state) return 0;

    state->kem = OQS_KEM_new(OQS_KEM_alg_hqc_128);
    ...
    state->parent.dh_id = NOISE_DH_HQC128;
    state->parent.nulls_allowed = 0;

    state->parent.private_key = state->private_key;
    state->parent.public_key  = state->public_key;

    state->parent.generate_keypair     = noise_hqc_generate_keypair;
    state->parent.set_keypair          = noise_hqc_set_keypair;
    state->parent.set_keypair_private  = noise_hqc_set_keypair_private;
    state->parent.validate_public_key  = noise_hqc_validate_public_key;
    state->parent.copy                 = NULL; 
    state->parent.calculate            = noise_hqc_calculate;
    state->parent.encaps               = noise_hqc_encapsulate;
    state->parent.decaps               = noise_hqc_decapsulate;
